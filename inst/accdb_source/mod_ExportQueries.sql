...
-- ParkSpeciesSQL(iPark As Integer) [L135]
...
-- LocTypeFilter(iPark As Integer) [L146]
...
-- TotalPointsSQL(iPark As Integer) [L202]
...
-- Export_AnnualReport_AbsoluteCover() [L1225]
...
SELECT qData.SurveyYear, qData.Park, qData.IslandCode, qData.SiteCode, qData.Vegetation_Community, qData.FxnGroup,
  qData.Nativity, qData.SumOfN, qTotalPoints.NofPoints,
  ([SumOfN]/[NofPoints])*100 AS AbsCover
FROM "strData" AS qData INNER JOIN
  "TotalPointsSQL(xPark)" AS qTotalPoints ON
  (qData.SurveyYear = qTotalPoints.SurveyYear) AND (qData.Park = qTotalPoints.Park) AND (qData.IslandCode = qTotalPoints.IslandCode) AND (qData.SiteCode = qTotalPoints.SiteCode)

---- strAbsCov =
SELECT qry.SurveyYear, "ParkSelect(xPark)", qry.Vegetation_Community, qry.FxnGroup, qry.Nativity,
  "Annual Report, Absolute Cover (Fig. E2)" AS Query_type,
  Count(qry.SiteCode) AS NofTransects,
  Avg(qry.AbsCover) AS Average,
  StDev(qry.AbsCover) AS StdDev,
  Min(qry.AbsCover) AS MinRange,
  Max(qry.AbsCover) AS MaxRange,
FROM "strAbsCovData" AS qry
